...
async def retry_interceptor(
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if context.allowed_mcp_tools and request.name not in context.allowed_mcp_tools:
                raise PermissionError(f"当前角色无权调用工具: {request.name}")
            ...
            metrics_service.increment("mcp_tool_success_total")
            metrics_service.observe("mcp_tool_latency", latency_ms)
            session_service.log_tool_call(
                ...
                input_payload=str(request.args),
            ...
            latency_ms = int((perf_counter() - started_at) * 1000)
            context = get_request_context()
            metrics_service.increment("mcp_tool_failure_total")
            metrics_service.observe("mcp_tool_latency", latency_ms)
            session_service.log_tool_call(
                session_id=context.session_id,
                workflow_run_id=context.workflow_run_id,
