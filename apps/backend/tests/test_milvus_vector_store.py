...
def test_importing_vector_store_package_does_not_import_pymilvus() -> None:
    sys.modules.pop("opspilot.vector_store", None)
    sys.modules.pop("opspilot.vector_store.milvus", None)
    sys.modules.pop("pymilvus", None)

    importlib.import_module("opspilot.vector_store")

    assert "pymilvus" not in sys.modules


def test_default_settings_and_explicit_project_config(tmp_path: Path) -> None:
    from opspilot.vector_store import load_milvus_vector_store_settings
...
def test_collection_schema_contains_required_fields_and_dimension() -> None:
    from opspilot.vector_store import MilvusVectorStoreSettings, build_chunk_collection_schema
...
def test_connection_manager_creates_client_only_when_explicitly_connected() -> None:
    from opspilot.vector_store import MilvusConnectionManager, MilvusVectorStoreSettings
...
async def test_ready_endpoint_reports_milvus_readiness_without_startup_connection(
    ...
    import opspilot.api.app as api_app
    from opspilot.api.app import create_app
    from opspilot.llm import LlmProvider, LlmReadinessResult
    from opspilot.vector_store import MilvusHealthCheckResult
