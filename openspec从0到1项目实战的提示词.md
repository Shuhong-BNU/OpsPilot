...
后端包必须位于 apps/backend/src/opspilot，只允许 from opspilot...，禁止从 src.opspilot 导入。模块 import 期间不得连接 SQLite、Milvus、LLM 或 MCP。

建立最小可运行骨架和质量命令：
- backend pyproject、uv.lock、opspilot 包、最小 /health app factory、pytest/Ruff/Pyright 配置。
...
- engine/session 只在 FastAPI lifespan、依赖 provider 或显式初始化路径创建；import opspilot.memory 不得打开数据库或运行迁移。
- 领域服务依赖 Repository Protocol/不可变 record，不得接收 ORM model；SQLite 实现位于 `opspilot.memory.sqlite`/`extended_sqlite`，为未来 PostgreSQL 保留替换边界。
...
Milvus adapter 要求：
- 使用 pymilvus 官方 client，但 import opspilot.vector_store 不得创建 client、连网或初始化 collection。
