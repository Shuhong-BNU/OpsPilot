...
后端使用 Python、`uv` 和 src 布局。应用代码位于 `src/opspilot` 下，并通过包名称进行导入：
...
uv run uvicorn opspilot.api:create_app --factory --reload
```

## LLM 提供者

后端在 `src/opspilot/llm` 下使用可替换的 LLM 提供商抽象。
...
```python
from opspilot.llm import build_default_llm_provider
...
`opspilot.memory.repositories`。SQLite 特定的代码位于
`opspilot.memory.sqlite`。

## Milvus 向量存储

Milvus 向量存储位于 `src/opspilot/vector_store` 下。导入 Milvus
