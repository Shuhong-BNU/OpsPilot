...
**目标：**
- 添加一个后端 `opspilot.vector_store` 包，包含配置、集合模式、连接管理器和仓库风格的 Milvus 向量存储抽象。
...
1. **使用专用的向量存储包，而不是将Milvus混入内存仓库中。**
   SQLite 内存层负责聊天、AIOps、认证和审计持久化。Milvus 负责相似性搜索。将`opspilot.vector_store` 保持独立可以保留更换Milvus 或存储后端的能力，而无需更改内存仓库契约。
