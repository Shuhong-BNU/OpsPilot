...
AUTH_LOGIN_API = $(SERVER_URL)/api/auth/login
DOCS_DIR = aiops-docs
...
help:
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo "$(GREEN)  OpsPilot - Makefile 命令$(NC)"
	@echo "$(GREEN)═══════════════════════════════════════════════════════$(NC)"
	@echo ""
	@echo "$(CYAN)【一键操作】$(NC)"
...
upload:
	...
	@auth_token=$$(curl -s -X POST $(AUTH_LOGIN_API) \
		-H "Content-Type: application/json" \
		-d '{"username":"operator","password":"operator123"}' | python3 -c "import sys, json; print(json.load(sys.stdin).get('access_token',''))" 2>/dev/null); \
	if [ -z "$$auth_token" ]; then \
		echo "$(RED)❌ 未获取到 operator 登录令牌，无法上传文档$(NC)"; \
		exit 1; \
	fi; \
	count=0; \
	success=0; \
...
test-upload:
	...
	if [ -n "$$first_file" ]; then \
		...
		curl -X POST $(UPLOAD_API) \
			...
			curl -X POST $(UPLOAD_API) -H "Authorization: Bearer $$auth_token" -F "file=@$$first_file"; \
	else \
		echo "$(RED)测试文件不存在$(NC)"; \
	fi
