...
if errorlevel 1 exit /b 1
start "OpsPilot CLS MCP" /D "%CD%" cmd /c "cls-mcp-server > var\cls-mcp-server-local.log 2>&1"
start "OpsPilot Backend" /D "%CD%" cmd /c "uv run uvicorn opspilot.api.app:create_app --factory --host 127.0.0.1 --port 8000 > var\backend-local.log 2>&1"
popd

pushd apps\frontend
if not exist node_modules npm install
if errorlevel 1 exit /b 1
start "OpsPilot Frontend" /D "%CD%" cmd /c "npm run dev -- --host 127.0.0.1 > ..\backend\var\frontend-local.log 2>&1"
popd
