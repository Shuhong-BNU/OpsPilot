...
仓库 SHALL 为 macOS、Linux 和 Windows 提供了单独的中文安装指南，列出了本地开发所需的每个依赖项：Git、Docker、Node/npm、uv 以及官方的全局 `cls-mcp-server` 包。

#### Scenario: macOS developer reads setup guidance
- **WHEN** 开发者打开 macOS 安装指南
- **THEN** 它 MUST 提供面向 Homebrew 的安装命令和主机本地项目启动命令。

#### Scenario: Linux developer reads setup guidance
- **WHEN** 开发者打开 Linux 设置指南  
